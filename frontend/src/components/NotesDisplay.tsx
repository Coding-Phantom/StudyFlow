  ...
  return (
    <div className="space-y-5">
      ...
      {notes.common_mistakes.length > 0 && (
        <div className="card border-amber-200/60 bg-amber-50/60 p-4">
      ...
      <div className="flex items-center justify-between">
        <span className="inline-flex items-center gap-1.5 text-xs bg-gray-100 text-gray-500 px-3 py-1.5 rounded-full font-medium">
        ...
        {onStartQuiz && (
          <button
            ...
            className="inline-flex items-center gap-1.5 text-xs bg-primary text-white px-4 py-1.5 rounded-lg font-semibold hover:bg-primary-dark transition-all duration-200 shadow-sm"
          >
            <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
      ...
      <div className="border-t border-gray-200/80 pt-5">
        ...
        <div className="flex gap-2">
          <input
            ...
            className="flex-1 px-3.5 py-2.5 border border-gray-300 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500/30 focus:border-indigo-500 transition-all duration-200 placeholder:text-gray-400"
            onKeyDown={(e) => e.key === "Enter" && handleAsk()}
          />
          <button
            onClick={handleAsk}
            disabled={explaining || !question.trim()}
            className="bg-primary text-white px-5 py-2.5 rounded-xl text-sm font-semibold hover:bg-primary-dark disabled:opacity-50 disabled:cursor-not-allowed transition-all duration-200 shadow-sm"
          >
            {explaining ? "..." : "Ask"}
          </button>
        </div>
        ...
        {explanation && (
          <div className="mt-3 bg-gray-50/80 rounded-xl p-4 text-sm text-gray-700 leading-relaxed border border-gray-100">
            {explanation}
          </div>
        )}
      </div>
