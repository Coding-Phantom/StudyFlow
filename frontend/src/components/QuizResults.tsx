...
export default function QuizResults({
  ...
  const intensityColors: Record<string, string> = {
    light: "bg-success/20 text-success border border-success/20",
    normal: "bg-info/20 text-info border border-info/20",
    intense: "bg-danger/20 text-danger border border-danger/20",
  };
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="card p-8 text-center">
        <div className="relative inline-flex items-center justify-center">
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center">
            <span className="text-3xl font-extrabold text-text-primary">{scorePercent}%</span>
            <span className="text-sm text-text-secondary font-medium">{adjustedScore}/{questions.length} correct</span>
          </div>
        </div>
          ...
          <div className="mt-6">
            ...
            <div className="flex flex-wrap justify-center gap-2">
              {result.weak_concepts.map((concept, i) => (
                <span
                  key={i}
                  className="inline-flex items-center gap-1.5 text-xs font-medium bg-warning/10 text-warning px-3 py-1.5 rounded-full border border-warning/20"
                >
      ...
      {/* AI Disclaimer */}
      <div className="flex items-start gap-2.5 text-sm bg-warning/10 border border-warning/20 rounded-xl px-4 py-3">
      ...
      <div>
        <div className="flex items-center gap-2 mb-4">
          ...
          <h3 className="text-lg font-bold text-text-primary">Question Breakdown</h3>
          {overriddenCorrect.size > 0 && (
            <span className="text-xs font-semibold text-warning bg-warning/10 border border-warning/20 px-2.5 py-0.5 rounded-full ml-auto">
      ...
      <div className="card p-6">
        <div className="flex items-center gap-2 mb-5">
          ...
          <h3 className="text-lg font-bold text-text-primary">Recommended Next Steps</h3>
        </div>
          ...
          <div className="space-y-5">
            ...
            <div className="flex items-center gap-4 flex-wrap">
              <div className="flex items-center gap-2">
                <span className="text-sm text-text-secondary">Study intensity:</span>
                <span
                  className={`text-xs font-bold px-3 py-1 rounded-full ${
                    intensityColors[adaptation.suggested_intensity] ?? "bg-white/[0.04] text-text-muted border border-border"
                  }`}
                >
            ...
            <div className="bg-accent/5 rounded-xl p-4 border border-accent/10">
              <p className="text-xs font-semibold text-accent uppercase tracking-wider mb-1">Next Focus</p>
              <p className="text-sm font-bold text-text-primary">{adaptation.next_focus}</p>
            </div>

            {/* Review topics */}
            {adaptation.review_topics.length > 0 && (
              <div>
                <p className="text-xs font-semibold text-text-muted uppercase tracking-wider mb-2">Review These Topics</p>
            ...
            <div className="bg-surface/50 rounded-xl p-4 border border-border">
              <p className="text-xs font-semibold text-text-muted uppercase tracking-wider mb-1.5">Why?</p>
              <p className="text-sm text-text-secondary leading-relaxed">{adaptation.reasoning}</p>
            </div>
